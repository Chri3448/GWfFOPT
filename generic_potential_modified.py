...
import transitionFinder_modified
...
class generic_potential():
    ...
    def getPhases(self,tracingArgs={}):
        ...
        phases = transitionFinder_modified.traceMultiMin(
            self.Vtot, self.dgradV_dT, self.d2V, points,
            tLow=0.0, tHigh=tstop, deltaX_target=100*self.x_eps,
            **tracingArgs_)
        self.phases = phases
        transitionFinder_modified.removeRedundantPhases(
    ...
    def calcTcTrans(self, startHigh=False):
        ...
        if self.phases is None:
            self.getPhases()
        self.TcTrans = transitionFinder_modified.findCriticalTemperatures(
    ...
    def findAllTransitions(self, tunnelFromPhase_args={}):
        ...
        if self.phases is None:
            self.getPhases()
        self.TnTrans = transitionFinder_modified.findAllTransitions(
            self.phases, self.Vtot, self.gradV, tunnelFromPhase_args)
        # Add in the critical temperature
        if self.TcTrans is None:
            self.calcTcTrans()
        transitionFinder_modified.addCritTempsForFullTransitions(
