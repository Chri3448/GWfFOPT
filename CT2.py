...
def get_thermal_params(input_params):
    ...
    '''
    Tnucs = np.zeros(input_params['f'].shape)
    betaHs = np.zeros(input_params['f'].shape)
    ksis = np.zeros(input_params['f'].shape)
    
    # we want to capture the output since CT really spits out a lot of info
    progress = 0
    total = input_params['f'].size
    ...
    for i, b in enumerate(input_params['b']):
        ...
        for j, c in enumerate(input_params['c']):
            ...
            if c/(b**2) < 1:
                ...
                continue
            param_instance['width'] = input_params['width'][i,j]
            
            # loop over f vals
            # note that our thermal parameter matrices will now be filled in, e.g., like ksis[i, j, k] = ...
            for k, f in enumerate(input_params['f'][i,j,:]):
                progress += 1
                if progress != total:
                    print('processing b, c, and f combination ', progress, '/', total, end = '\r')
                else:
                    print('processing b, c, and f combination ', progress, '/', total)
                param_instance['f'] = f
...
print('begining first pass over parameters')
...
for Lambda in Lamvals:
    ...
    for c1 in c1vals:
        ...
        input_params['c1'] = c1
        print('Lambda = ', Lambda, ', c1 = ', c1)
        Tnucs, betaHs, ksis = get_thermal_params(input_params)
        c1list.append({'Tnucs': Tnucs, 'betaHs': betaHs, 'ksis': ksis})
...
if recalc:
    print('begining recalculation pass over parameters')
    for Li, Lambda in enumerate(Lamvals):
        for c1i, c1 in enumerate(c1vals):
            print('Lambda = ', Lambda, ', c1 = ', c1)
            progress = 0
            total = np.where(np.logical_or(np.logical_or(thermal_params[Li,c1i]['Tnucs']==0, thermal_params[Li,c1i]['betaHs']==0), thermal_params[Li,c1i]['ksis']==0))[0].size
            for bi, b in enumerate(bvals):
                for ci, c in enumerate(cvals):
                    for fi, f in enumerate(fvals[bi,ci,:]):
                        if np.any((thermal_params[Li,c1i]['Tnucs'][bi,ci,fi]==0, 
                                   thermal_params[Li,c1i]['betaHs'][bi,ci,fi]==0, 
                                   thermal_params[Li,c1i]['ksis'][bi,ci,fi]==0)) and c/(b**2) >= 1 or recalc_all:
                            progress += 1
                            if progress != total:
                                print('reprocessing b, c, and f combination ', progress, '/', total, end = '\r')
                            else:
                                print('reprocessing b, c, and f combination ', progress, '/', total)
                            input_params['Lamda'] = Lambda
                            input_params['c1'] = c1
                            ...
                            input_params['f'] = np.tile(f, (recalc_resolution, recalc_resolution, 1))
                            input_params['width'] = np.tile(widthvals[bi,ci], (recalc_resolution, recalc_resolution))
                            with HiddenPrints():
                                Tnucs, betaHs, ksis = get_thermal_params(input_params)
                            ...
                            if ksis_mean == np.nan:
                                ksis_mean = 0
                            thermal_params[Li,c1i]['Tnucs'][bi,ci,fi] = Tnucs_mean
                            thermal_params[Li,c1i]['betaHs'][bi,ci,fi] = betaHs_mean
                            thermal_params[Li,c1i]['ksis'][bi,ci,fi] = ksis_mean
